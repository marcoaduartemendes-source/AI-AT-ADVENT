...
class CryptoBasisTrade(Strategy):
    ...
    def on_emergency_close(self, ctx):
        """KILL switch — close both legs (spot + dated future).
        Audit-fix F1 (2026-05-07): same ledger fallback as
        crypto_funding_carry; the future short leg is invisible to
        spot-only get_positions.
        """
